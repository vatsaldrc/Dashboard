...
export function BookingSankeyChartPlotly({
  ...
  const formatNodeLabel = (id: string): string => {
    const labelMap: Record<string, string> = {
      ...
      customer_type_business: "B2B",
      customer_type_private: "B2C",
  ...
  const getNodeColor = (id: string): string => {
    if (id === "booking_started") return "#3b82f6";
    if (id.startsWith("contact_method_")) return "#8b5cf6";
    if (id === "details_collected") return "#06b6d4";
    if (id.startsWith("customer_type_")) return "#f59e0b";
    if (id === "lead_created") return "#10b981";
    if (id === "booking_cancelled") return "#ef4444";
    if (id === "dropped") return "#6b7280";
    return "#64748b";
  };
  ...
  const plotlyData = useMemo(() => {
    ...
    const nodeIndexMap = new Map(data.nodes.map((node, idx) => [node.id, idx]));
    ...
    return [
      {
        ...
        orientation: "h",
        node: {
          pad: 60, // Vertical padding between nodes
        ...
        link: {
          ...
          color: linkColors,
          hovertemplate:
            "%{source.label} → %{target.label}<br>Users: %{value}<extra></extra>",
        },
      },
    ];
  }, [data]);
